   ...
   <div class="popup-body">
    ...
    <div class="info-message" v-if="!loading">
     ...
     <div v-else-if="tournamentTeams.length < 2" class="info-card error">
      ...
      <p>Se necesitan al menos 2 equipos para generar un cronograma. Actualmente hay {{ tournamentTeams.length }} equipo
       asignado.</p>
     </div>
       ...
       <div v-for="(groupFixtures, groupName) in groupedFixtures" :key="groupName" class="group-section">
        ...
        <div v-for="(fixture, index) in groupFixtures" :key="`${groupName}-${index}`" class="fixture-card"
         ...
         <div class="fixture-header">
          ...
          <button @click="moveFixture(fixtures.indexOf(fixture), -1)" :disabled="fixtures.indexOf(fixture) === 0"
           class="move-btn up">⬆</button>
         ...
         <div class="fixture-content">
          ...
          <div class="fixture-details">
           ...
           <div class="input-group">
            <label>Sede</label>
            <input type="text" v-model="fixture.venue" placeholder="Sede del partido" class="form-input-small" />
           </div>
          </div>
         </div>
        </div>
...
const tournamentTeams = computed(() => {
 console.log('🔍 Calculando equipos del torneo...')
 console.log('TournamentData:', props.tournamentData)
 console.log('TeamAssignments directo:', props.tournamentData?.teamAssignments)
 console.log('Groups:', props.tournamentData?.groups)
 console.log('AllTeams cargados:', allTeams.value.length, allTeams.value)
 ...
 if (props.tournamentData?.teamAssignments && props.tournamentData.teamAssignments.length > 0) {
  ...
  if (realTeams.length > 0) {
   ...
   console.log('🚀 Equipos finales con grupos:', formattedTeams)
   return formattedTeams
  }
 }

 console.log('❌ No hay teamAssignments o no se cargaron equipos')
 return []
})
...
const generateFixtures = () => {
 console.log('🔄 Generando fixtures...')
 console.log('Equipos disponibles:', tournamentTeams.value)
 console.log('Tipo de fixture:', fixtureType.value)

 if (tournamentTeams.value.length < 2) {
  console.log('❌ Menos de 2 equipos disponibles')
  return
 }

 fixtures.value = []
 console.log('✅ Limpiando fixtures existentes')

 if (fixtureType.value === 'group_stage') {
  console.log('🔄 Generando fixtures por Grupos')
  generateGroupStageFixtures()
 } else if (fixtureType.value === 'round_robin') {
  console.log('🔄 Generando fixtures Round Robin')
  generateRoundRobinFixtures()
 } else {
  console.log('🔄 Generando fixtures Knockout')
  generateKnockoutFixtures()
 }

 console.log('📅 Actualizando fechas')
 updateDates()
 console.log('✅ Fixtures generados:', fixtures.value.length)
}
...
const loadExistingFixtures = async () => {
 ...
 try {
  console.log('🔍 Buscando matches existentes para torneo:', props.tournamentData.id)
  ...
  if (existingMatches && existingMatches.length > 0) {
   ...
   console.log('🏆 Fixtures cargados para UI:', fixtures.value.length)
   console.log('📋 Fixtures procesados:', fixtures.value.map(f => ({
    id: f.id,
   ...
   if (fixtures.value.length > 0) {
    ...
    // El tipo de fixture ya se detecta automáticamente por el watcher de grupos
    console.log('📅 Configuración inferida - Fecha:', startDate.value, 'Sede:', venue.value)
   }
  } else {
   ...
   console.log('📋 Configurados valores por defecto.')
   console.log('👤 El usuario puede usar "Regenerar Cronograma" para crear fixtures.')
  }
 } catch (error) {
  console.error('⚠️ Error cargando matches:', error)
  // Si hay error, solo configurar valores por defecto
  const today = new Date()
  startDate.value = today.toISOString().split('T')[0]
  venue.value = 'Estadio Principal'

  console.log('⚠️ Error cargando matches. Configurados valores por defecto.')
  console.log('👤 El usuario puede usar "Regenerar Cronograma" para crear fixtures.')
 } finally {
  loading.value = false
 }
...
// Watcher para regenerar cuando cambie el torneo (ÚNICO punto de carga)
watch(() => props.tournamentData, async (newTournament) => {
 if (newTournament) {
  console.log('🔄 Cargando datos del torneo:', newTournament.name)
  await loadTeams()
...
watch(() => props.tournamentData?.groups?.length, (numberOfGroups) => {
 if (numberOfGroups !== undefined) {
  ...
  console.log(`🔄 Tipo de fixture detectado automáticamente: ${fixtureType.value} (${numberOfGroups} grupos)`)
 }
}, { immediate: true })
</script>
